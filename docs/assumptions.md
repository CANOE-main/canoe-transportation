...
| LD and MHD EV Chargers | NRCan/Dunsky — unaccounted existing private charger capacity | - Dunsky assumes 1 LD EV per port and 1.5 MHD per port; this ratio is used to find remaining private chargers<br>- NRCan reports the total cumulative LD chargers by province and national type (L1, L2, L3) shares<br>- It is assumed that the entirety of MHDV chargers are covered through their ratio, distributed with Dunsky type (100 kW, 350kW) share assumptions. |
| LDEV Chargers | Transport Canada — undisclosed capacity vintage | Assuming all capacity is from last existing period. |
