...
# Function to update Commodity and Demand tables based on keys and parameter
...
# Function to replace rows based on 'tech' matching any 'tech' from Technology table in the source db
def replace_tech_rows(table_name):
    # Get the list of techs from the source Technology table
    source_cursor.execute('SELECT tech FROM "Technology"')
    techs = [row[0] for row in source_cursor.fetchall()]
    
    # Delete rows where 'tech' matches any tech from the source Technology table
    for tech in techs:
...
for table_name in ['LifetimeTech', 'ExistingCapacity', 'CapacityToActivity', 'MaxAnnualCapacityFactor', 'MinAnnualCapacityFactor', 'Efficiency', 'CostInvest', 'CostFixed', 'CostVariable', 'EmissionActivity', 'TechInputSplit']:
